...
void AHunter::Tick(float DeltaTime)
{
	...
	//and then change the state
		...
		decision = neuroDecide(prayTarget, mateTarget, isMating, isOvertime());
		//make the expected decision
		double ActualDecision = actualDecision(decision, prayTarget, mateTarget, isMating, isOvertime());
		//and adjust the neuro network after realise
		DNAController->adjustDecision(ActualDecision);
...
double AHunter::actualDecision(double inDecision, ACreature * inPray, AHunter * inMate, bool isMating, bool isOvertime)
{
...
double AHunter::adjustDecision(double inDecision, ACreature * inPray, AHunter * inMate, bool isMating, bool isOvertime)
{
	int outDecision=0;
	//state id: 0:wander, 1:chase, 2:toMate
	int decision = round(inDecision);

	//if there's no mate or got mate but overtime, keep it to wander as it's unable to mate, otherwise keep the decision
	if (decision == 2) {
		if (inMate != nullptr && !isOvertime) {
			outDecision = inDecision;
		}
	}
	//if there's no pray or got pray but overtime, keep it to wander as it's unable to mate, otherwise keep the decision
	else if (decision == 1) {
		if (inPray != nullptr && !isOvertime) {
			outDecision = inDecision;
		}
